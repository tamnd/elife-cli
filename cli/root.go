// Package cli builds the elife command tree on top of the elife library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: elife.DefaultConfig()}

	root := &cobra.Command{
		Use:   "elife",
		Short: "Browse eLife Sciences open-access journal",
		Long: `elife reads eLife Sciences open-access articles through the public API.
No credentials required. Returns records as table, JSON, JSONL, CSV, TSV, URL, or raw.

elife is an independent tool and is not affiliated with eLife Sciences.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = elife.NewClient(a.cfg)
	return nil
}
