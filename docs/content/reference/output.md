...
describes what users get. Pick a format with `-o`, or let elife choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
elife <command> -o table   # aligned columns for reading
elife <command> -o jsonl   # one JSON object per line, for piping
elife <command> -o json    # a single JSON array
elife <command> -o csv     # spreadsheet friendly
elife <command> -o tsv     # tab-separated
elife <command> -o url     # just the URL column
elife <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
elife <command> --fields id,title,url
```
...
```bash
elife <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
elife <command>            # a table, because this is a terminal
elife <command> | wc -l    # JSONL, because this is a pipe
```
